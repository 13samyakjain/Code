...
public:
    ...
    int bfs(vector<vector<int>>& grid){ // finding the farthest '0' by bfs
        ...
        while (!q.empty()){
            ...
            while(n-- > 0){ // going through every level
                auto a = q.front();
                q.pop();
                ...
                for (int h=0; h<4; h++) {
                    ...
                    if (x>=0 and x<grid.size() and y>=0 and y<grid[0].size() and grid[x][y] == 0) { // if the neighbor is 0, then mark it visited and add it to queue and storing maximum distance till that cordinate
    ...
    int maxDistance(vector<vector<int>>& grid) {
        ...
        for (int i=0; i<grid.size(); i++){
            for (int j=0; j<grid[0].size(); j++){
                if (grid[i][j] == 1){ // found 1st '1' in the matrix
